#!/usr/bin/env zeek
module IMAPSPacketCountBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="IMAPSPacketCountBruteForce packet observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting IMAPSPacketCountBruteForce packets", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["IMAPSPacketCountBruteForce packet observed"]$sum;
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("IMAPSPacketCountBruteForce packet observed",
