#!/usr/bin/env zeek
module IMAPSSimilarSizedResponsesBruteForce;
export {
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("IMAPSSimilarSizedResponsesBruteForce response size observed",
...
event zeek_init() &priority=15 {
  local reducer = SumStats::Reducer($stream="IMAPSSimilarSizedResponsesBruteForce response size observed", $apply=set(SumStats::TOPK));

  SumStats::create([
    $name = "counting IMAPSSimilarSizedResponsesBruteForce response sizes", $epoch=epoch_len, $reducers=set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local topk = result["IMAPSSimilarSizedResponsesBruteForce response size observed"]$topk;
