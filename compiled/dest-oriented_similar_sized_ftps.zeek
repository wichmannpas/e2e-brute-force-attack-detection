#!/usr/bin/env zeek
module FTPSDestOrientedSimilarSizedResponsesBruteForce;
export {
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("FTPSDestOrientedSimilarSizedResponsesBruteForce response size observed",
...
event zeek_init() &priority=15 {
  local reducer = SumStats::Reducer($stream="FTPSDestOrientedSimilarSizedResponsesBruteForce response size observed", $apply=set(SumStats::TOPK));

  SumStats::create([
    $name = "counting FTPSDestOrientedSimilarSizedResponsesBruteForce response sizes", $epoch=epoch_len, $reducers=set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local topk = result["FTPSDestOrientedSimilarSizedResponsesBruteForce response size observed"]$topk;
