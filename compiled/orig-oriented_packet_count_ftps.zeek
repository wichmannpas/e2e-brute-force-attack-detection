#!/usr/bin/env zeek
module FTPSPacketCountBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="FTPSPacketCountBruteForce packet observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting FTPSPacketCountBruteForce packets", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["FTPSPacketCountBruteForce packet observed"]$sum;
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("FTPSPacketCountBruteForce packet observed",
