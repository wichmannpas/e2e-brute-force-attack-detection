...
module MAINConnectionCountBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="MAINConnectionCountBruteForce connection observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting MAINConnectionCountBruteForce connections", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["MAINConnectionCountBruteForce connection observed"]$sum;
...
event connection_state_remove(c: connection) {
  ...
  SumStats::observe("MAINConnectionCountBruteForce connection observed",
