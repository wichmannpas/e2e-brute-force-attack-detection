#!/usr/bin/env zeek
module FTPSConnectionCountBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="FTPSConnectionCountBruteForce connection observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting FTPSConnectionCountBruteForce connections", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["FTPSConnectionCountBruteForce connection observed"]$sum;
...
event connection_state_remove(c: connection) {
  ...
  SumStats::observe("FTPSConnectionCountBruteForce connection observed",
