#!/usr/bin/env zeek
module FTPSDestOrientedShortConnectionsBruteForce;
export {
  # The (human-readable) name of the protocol
  const proto = "FTPS";

  # The port of the service
  const service_port = 21/tcp;
}
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="FTPSDestOrientedShortConnectionsBruteForce short connection duration observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting FTPSDestOrientedShortConnectionsBruteForce short connection durations", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["FTPSDestOrientedShortConnectionsBruteForce short connection duration observed"]$sum;
...
event connection_state_remove(c: connection) {
  ...
  SumStats::observe("FTPSDestOrientedShortConnectionsBruteForce short connection duration observed",
