#!/usr/bin/env zeek
module SSHShortConnectionsBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="SSHShortConnectionsBruteForce short connection duration observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting SSHShortConnectionsBruteForce short connection durations", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["SSHShortConnectionsBruteForce short connection duration observed"]$sum;
...
event connection_state_remove(c: connection) {
  ...
  SumStats::observe("SSHShortConnectionsBruteForce short connection duration observed",
