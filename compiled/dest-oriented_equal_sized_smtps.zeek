#!/usr/bin/env zeek
module SMTPSDestOrientedEqualSizedResponsesBruteForce;
export {
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("SMTPSDestOrientedEqualSizedResponsesBruteForce response size observed",
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="SMTPSDestOrientedEqualSizedResponsesBruteForce response size observed", $apply=set(SumStats::TOPK));

  SumStats::create([
    $name = "counting SMTPSDestOrientedEqualSizedResponsesBruteForce response sizes", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local topk = result["SMTPSDestOrientedEqualSizedResponsesBruteForce response size observed"]$topk;
