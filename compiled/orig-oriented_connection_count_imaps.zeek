#!/usr/bin/env zeek
module IMAPSConnectionCountBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="IMAPSConnectionCountBruteForce connection observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting IMAPSConnectionCountBruteForce connections", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["IMAPSConnectionCountBruteForce connection observed"]$sum;
...
event connection_state_remove(c: connection) {
  ...
  SumStats::observe("IMAPSConnectionCountBruteForce connection observed",
