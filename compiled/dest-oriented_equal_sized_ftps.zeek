#!/usr/bin/env zeek
module FTPSDestOrientedEqualSizedResponsesBruteForce;
export {
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("FTPSDestOrientedEqualSizedResponsesBruteForce response size observed",
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="FTPSDestOrientedEqualSizedResponsesBruteForce response size observed", $apply=set(SumStats::TOPK));

  SumStats::create([
    $name = "counting FTPSDestOrientedEqualSizedResponsesBruteForce response sizes", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local topk = result["FTPSDestOrientedEqualSizedResponsesBruteForce response size observed"]$topk;
