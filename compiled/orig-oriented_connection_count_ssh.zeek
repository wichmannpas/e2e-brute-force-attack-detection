#!/usr/bin/env zeek
module SSHConnectionCountBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="SSHConnectionCountBruteForce connection observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting SSHConnectionCountBruteForce connections", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["SSHConnectionCountBruteForce connection observed"]$sum;
...
event connection_state_remove(c: connection) {
  ...
  SumStats::observe("SSHConnectionCountBruteForce connection observed",
