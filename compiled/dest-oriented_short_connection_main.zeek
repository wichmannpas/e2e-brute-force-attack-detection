...
module MAINDestOrientedShortConnectionsBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="MAINDestOrientedShortConnectionsBruteForce short connection duration observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting MAINDestOrientedShortConnectionsBruteForce short connection durations", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["MAINDestOrientedShortConnectionsBruteForce short connection duration observed"]$sum;
...
event connection_state_remove(c: connection) {
  ...
  SumStats::observe("MAINDestOrientedShortConnectionsBruteForce short connection duration observed",
