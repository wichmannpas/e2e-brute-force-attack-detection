#!/usr/bin/env zeek
module SMTPSSimilarSizedResponsesBruteForce;
export {
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("SMTPSSimilarSizedResponsesBruteForce response size observed",
...
event zeek_init() &priority=15 {
  local reducer = SumStats::Reducer($stream="SMTPSSimilarSizedResponsesBruteForce response size observed", $apply=set(SumStats::TOPK));

  SumStats::create([
    $name = "counting SMTPSSimilarSizedResponsesBruteForce response sizes", $epoch=epoch_len, $reducers=set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local topk = result["SMTPSSimilarSizedResponsesBruteForce response size observed"]$topk;
