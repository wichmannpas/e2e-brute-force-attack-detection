#!/usr/bin/env zeek
module SSHPacketCountBruteForce;
...
event zeek_init() &priority=5 {
  local reducer = SumStats::Reducer($stream="SSHPacketCountBruteForce packet observed", $apply=set(SumStats::SUM));

  SumStats::create([
    $name = "counting SSHPacketCountBruteForce packets", $epoch = epoch_len, $reducers = set(reducer),
    $epoch_result(ts: time, key: SumStats::Key, result: SumStats::Result) = {
      local sum = result["SSHPacketCountBruteForce packet observed"]$sum;
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
  ...
  SumStats::observe("SSHPacketCountBruteForce packet observed",
